...
proc nimbleDump*(self: c.Config): pc.Config =
    ...
    self.debug("Looking for nimble config in: ", self.sourcePath)
    return self.requireCaptureSh(self.sourcePath, self.requireExe("nimble"), [ "dump" ]).apply(it):
...
proc nimbleBin*(self: c.Config): tuple[absPath, importable: string] =
    ...
    let absPath = self.sourcePath / importable & ".nim"
